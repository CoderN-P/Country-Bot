...
[{len(self.bot.commands)}]```''')
  ...
  async def wiki(self, ctx, *, page: str):
    try:
        ...
        await ctx.send(embed=discord.Embed(title=data.title, url=data.url, description=data.summary.split('\n')[0]))
    except wikipedia.exceptions.DisambiguationError as e:
        await ctx.send(embed = discord.Embed(title='Did you mean', description=', '.join(e.options)))

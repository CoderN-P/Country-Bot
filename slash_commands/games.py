...
class EconomyCommands2(commands.Cog, name='Economy Commands (slash)', description='Commands that let you have your own little economy system'):
  ...
  @cog_ext.cog_slash(description="Check information about your country, or another person's country.")
  async def profile(self, ctx, member: discord.Member=None):
    ...
    try:
      ...
      embed = discord.Embed(title='Hey!', description=f':x: You or the person you are viewing do not have a country! Create one with `/start`')
      await ctx.send(embed=embed)
  ...
  async def work(self, ctx):
    ...
    if a[0][1] >= 100 and a[0][3] == 'Mayor':
      ...
      await ctx.send(embed=embed)
      await update((ctx.author.id, a[0][0], a[0][1],a[0][2],'State Senator', a[0][4], a[0][10]))
      ...
      await ctx.send(embed=embed)
      await update((ctx.author.id, a[0][0], a[0][1],a[0][2],'Governor', a[0][4], a[0][10]))
      a = await reading(ctx.author.id)
      ...
      await ctx.send(embed=embed)
      await update((ctx.author.id, a[0][0], a[0][1],a[0][2],'Senator', a[0][4], a[0][10]))
      a = await reading(ctx.author.id)
      ...
      await ctx.send(embed=embed)
      await update((ctx.author.id, a[0][0], a[0][1],a[0][2],'Vice President', a[0][4], a[0][10]))
      a = await reading(ctx.author.id)
      ...
      await ctx.send(embed=embed)
      await update((ctx.author.id, a[0][0], a[0][1],a[0][2],'President', a[0][4], a[0][10]))
      a = await reading(ctx.author.id)
    ...
    try:
      if a[0][3] == 'Mayor':
        ...
        await update((ctx.author.id, a[0][0], amount,multi, 'Mayor', a[0][4], a[0][10] + 1))
        ...
        if chance == 5:
          a = await reading(ctx.author.id)
          await update((ctx.author.id, a[0][0], a[0][1], a[0][2] + 1, a[0][3], a[0][4], a[0][10]))
        ...
        amount = amount1 + int(a[0][1])
        multi = float("{:.1f}".format(a[0][2] + ((a[0][5] + 1)/10)))
        await update((ctx.author.id, a[0][0], amount,multi, 'State Senator', a[0][4], a[0][10] + 1))
        ...
        amount = amount1 + int(a[0][1])
        multi = float("{:.1f}".format(a[0][2] + ((a[0][5] + 1)/10)))
        await update((ctx.author.id, a[0][0], amount,multi, 'Governor', a[0][4], a[0][10] + 1))
        ...
        amount = amount1 + int(a[0][1])
        multi = float("{:.1f}".format(a[0][2] + ((a[0][5] + 1)/10)))
        await update((ctx.author.id, a[0][0], amount,multi, 'Senator', a[0][4], a[0][10] + 1))
        ...
        amount = amount1 + int(a[0][1])
        multi = float("{:.1f}".format(a[0][2] + ((a[0][5] + 1)/10)))
        await update((ctx.author.id, a[0][0], amount,multi, 'Vice President', a[0][4], a[0][10] + 1))
        ...
        amount = amount1 + int(a[0][1])
        multi = float("{:.1f}".format(a[0][2] + ((a[0][5] + 1)/10)))
        await update((ctx.author.id, a[0][0], amount,multi, 'President', a[0][4], a[0][10] + 1))
  ...
  @cog_ext.cog_slash(description='Your population and multiplier will be reset, but you earn more coins, multiplier and population.')
    ...
    if a[0][1] > a[0][6]:
      ...
      try:
          ...
          if msg.content.lower() == 'n':
            ...
            await update_prestige((ctx.author.id, a[0][0], 0, 1, 'Mayor', 1, a[0][5] + 1, (a[0][6] + 50000000)))
      ...
      embed = discord.Embed(title='Oh No', description=''':x: you haven't met the requirements to prestige. You can see the requirements on your profile! (under the prestige section)''')
      await ctx.send(embed=embed)
  ...
  @cog_ext.cog_slash(description='Buy items for your country! Check out the items for sale with the shop command!')
  async def buy(self, ctx, id: int, amount: int = None):
    ...
    if amount:
      ...
      if id == '1':
        ...
        await update((ctx.author.id, a[0][0], a[0][1], a[0][2], a[0][3], (a[0][4] + 1), a[0][10]))
        ...
        await update((ctx.author.id, a[0][0], a[0][1], a[0][2], a[0][3], (a[0][4] + 2), a[0][10]))
        ...
        await update((ctx.author.id, a[0][0], a[0][1], a[0][2], a[0][3], (a[0][4] + 5), a[0][10]))
  ...
  @cog_ext.cog_slash(description='Gift some population to other people! Specify "half" or "full" or an integer as the amount!')
      ...
      if amount.isnumeric():
        if int(amount) > b[0][1]:
          ...
          await ctx.send(embed=embed)
            ...
            await ctx.send(embed=discord.Embed(title="Success!", description=f"Succesfully transefered {amount} people to {user}'s country!"))
        ...
        if amount.lower() == 'half':
            ...
            await ctx.send(embed=discord.Embed(title="Success!", description=f"Succesfully transefered {int(b[0][1]/2)} people to {user}'s country!"))
            ...
            await ctx.send(embed=discord.Embed(title="Success!", description=f"Succesfully transefered {int(b[0][1])} people to {user}'s country!"))
  ...
  async def daily(self, ctx):
    ...
    await update((ctx.author.id, a[0][0], a[0][1] + int((100 * (((a[0][1]**0.5)/100)) * (a[0][5] +1))), a[0][2], a[0][3], a[0][4], a[0][10]))
...
class Games2(commands.Cog, description='Cool games that test your geography skills (slash)'):
  def __init__(self, bot):
    self.bot = bot
  ...
  async def war(self, ctx, user: discord.Member):
    ...
    await ctx.send(f"<@!{b}>, you have 20 seconds to accept <@!{ctx.author.id}> request to war. Type `accept` in the chat to accept, or type `deny` in the chat to end the conflict")
    ...
    while n:
      if a == 4:
        ...
        return
      await ctx.send(f"<@!{ctx.message.author.id}> how many troops do you want to deploy, type `quit` to end")
      try:
        msg = await self.bot.wait_for('message', check=check, timeout=20)
      except asyncio.TimeoutError:
      ...
      if num.is_integer():
        if int(msg.content) < user1[0][1]:
          ...
          await ctx.send(f":x: <@!{ctx.message.author.id}> you dont have enough people!!!!")
          a += 1
          continue
      else:
    ...
    while n:
      if a == 4:
        await ctx.send('You tried this too many times! quitting game...')
        return
      await ctx.send(f"{user} how many troops do you want to deploy, type `quit` to end")
      try:
        msg = await self.bot.wait_for('message', check=check, timeout=20)
      except asyncio.TimeoutError:
          await ctx.send("Time ran out. No war :(")
    ...
    await ctx.send(f"What is the capital of....... `{random_country.title()}` {lookup(result4)}")
    ...
    def check(m):
      ...
      return m.content.lower() == country_capital.lower() and m.channel == ctx.channel and int(m.author.id) in [int(b), int(ctx.author.id)]
    ...
    if msg.author == ctx.author:
        await ctx.channel.send(f'<@!{ctx.author.id}> you gave the answer first. You won the war!!! :crown:')
        await update_war((ctx.author.id, user1[0][0], user1[0][1] + user2_troops, user1[0][2], user1[0][3], user1[0][4], user1[0][5], user1[0][6], user1[0][7] + 1, user1[0][8] + 1, user1[0][9]))
        ...
        await update_war((ctx.author.id, user1[0][0], user1[0][1] - user1_troops, user1[0][2], user1[0][3], user1[0][4], user1[0][5], user1[0][6], user1[0][7] + 1, user1[0][8], user1[0][9] + 1))
  ...
  @cog_ext.cog_subcommand(base='guess', description='Guess the capitals of countries. This game lasts for 10 minutes.', name='capital')
  async def guess_capital(self, ctx):
      ...
      while time.monotonic() - start < 600:      
        ...
        try:
          ...
          try:
            await ctx.send(f'''Time has run out!! ***Game Over***. Total score: `{count}` 
  ...
  @cog_ext.cog_subcommand(base='guess', name='country', description='Guess countries from their capitals. This game lasts for 10 minutes.')
  async def reverse(self, ctx):
        ...
        while time.monotonic() - start < 600:
          ...
          await ctx.send(f"What Country has `{country_capital.title()}` as its capital. Here is a hint: {lookup(result4)}")
          ...
          def check(m):
            return m.content.lower() == 'quit' or fuzz.ratio(random_country.lower(), m.content.lower()) > 82 and m.channel == channel
